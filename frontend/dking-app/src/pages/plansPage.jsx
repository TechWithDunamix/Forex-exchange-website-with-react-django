        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan, index) => (
            <Card key={index} title={plan.name} bordered={false} className="shadow-lg">
              ...
              <p className="text-green-800">Duration: {plan.duration}</p>
              <ul className="mb-4">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="mb-2">{feature}</li>
                ))}
              </ul>
              <a href={`/buy-plan?id=${plan.id}&int=${plan.pir}&period=${plan.duration}&name=${plan.name}&amount=${plan.price}`} className="hover:mr-20 hover:text-white w-full bg-green-600 px-3 py-2 rounded-b-lg rounded-r-lg">Buy Now</a>
            </Card>
          ))}
        </div>
      </div>
