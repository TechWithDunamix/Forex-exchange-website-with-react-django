import { LockOutlined, MailOutlined, MessageFilled } from "@ant-design/icons";
...
const SetPasswordPage = () => {
	...
	const requestOTP = async () => {
		...
		const request = await callMainApi("/otp/get", {
			method: "POST",
			body: { purpose: "set_password", email: formData.email },
		});
		if (!request.error) {
	...
	return (
		<div className="min-h-screen flex items-center justify-center bg-slate-900">
			<div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md mx-5">
				<h2 className="text-2xl font-bold text-center mb-8 text-green-600">Set Password</h2>
					...
					<div className="flex justify-between mt-10">
						...
						{!otp && (
							<button
								type="button"
								onClick={requestOTP}
								className="items-center flex hover:bg-green-600 bg-green-700 text-slate-200 p-2 rounded-lg drop-shadow-sm"
							>
								<MessageFilled />
								{isLoading ? (
									<div className="mx-12 p-1 animate-spin border-t-4 border-green-100 w-[20px] h-[20px] rounded-full"></div>
								) : (
									"Request OTP"
								)}
							</button>
					...
					{confirmed && (
						<>
							<div className="relative mt-4">
								...
								/>
								{formErrors.password && (
									<p className="text-red-500 font-light">{formErrors.password}</p>
								)}
							</div>
							...
							<button
								type="submit"
								className="w-full py-2 px-4 bg-green-600 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-600 mt-6"
							>
								Set Password
							</button>
						</>
