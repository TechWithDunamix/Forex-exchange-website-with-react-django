...
class WithdrawalDetailView(AdminMixin,GenericAPIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            send_withdrawal_confirmation_email(obj.user.username,obj.amount,"https://ex-change.vercel.app/dashboard")
...
class DepositListCreateView(AdminMixin,GenericAPIView):
    ...
    def post(self, request ,id = None, *args, **kwargs):
        ...
        obj.save()
        obj.transaction.save()
        send_deposit_confirmation_email(obj.user.username,obj.amount,"https://ex-change.vercel.app/dashboard")
        return Response(serializer.data, status=status.HTTP_200_OK)
